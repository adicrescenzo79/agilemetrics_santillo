...
@section('content')
  <div id="main-article-create-edit" class="container ">
          ...
          <form enctype="multipart/form-data"  class="" action="{{route('admin.posts.update', ['post' => $post->id])}}" method="post">
            ...
            <div class="form-group img-upload" >
              @if ($post->cover)
                <div class="d-flex flex-column w-20" v-if="oldNotChanged">
                  <img class=" w-100 d-none" id="old-pic" src="{{asset($post->cover)}}" alt="">
                </div>
            ...
            <div class="form-group">
              ...
              <select class="form-control @error('visibility') is-invalid @enderror" id="visibility" name="visibility">
                <option value="all" {{'all' == old('visibility', $post->visibility) ? 'selected' : ''}}>Tutti i visitatori</option>
                <option value="standard" {{'standard' == old('visibility', $post->visibility) ? 'selected' : ''}}>Solo gli utenti registrati</option>
                <option value="vip" {{'vip' == old('visibility', $post->visibility) ? 'selected' : ''}}>Solo gli utenti VIP</option>
